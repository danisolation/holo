...
**Shipped:** v1.0 (2026-04-15), v1.1 (2026-04-17), v2.0 (2026-04-17), v3.0 (2026-04-20), v4.0 (2025-07-20), v9.0 (2026-05-04), v10.0 (2026-05-05), v11.0 (2026-05-05), v12.0 (2026-05-05), v13.0 (2026-05-05), v17.0, v24.0 (2026-05-15)
**Code:** ~30,000+ LOC (Python + TypeScript)
**Tests:** 532 backend unit tests
**Stack:** FastAPI + PostgreSQL + APScheduler + WebSocket + Google Gemini + python-telegram-bot + Next.js + lightweight-charts + Recharts

## Current Milestone: v26.0 Dashboard UX & DevOps

**Goal:** Nâng cấp trải nghiệm frontend (dark mode, mobile, chart annotations, animations) và thiết lập hệ thống CI/CD + monitoring cho production stability.

**Target features:**
- Dark mode + theme toggle (next-themes)
- Mobile responsive optimization toàn bộ dashboard
- Chart annotations — vẽ entry/SL/TP trên lightweight-charts
- Loading states + smooth animations (skeleton, transitions)
- CI/CD pipeline — GitHub Actions: test → build → deploy
- Health alerts — Telegram thông báo khi service down
- Uptime monitoring — ping + response time tracking
- Error tracking tập trung — structured logging
...
- ✓ AIUP-01 through CTX-03: AI accuracy tracking, rumor-to-signal integration, enhanced Gemini context — v13.0

- ✓ CRAWL-01 through PERF-02: Performance optimization (bulk inserts, parallel crawlers, async patterns) — v15.0
- ✓ WS-01 through FE-03: Real-time price (VNDirect WebSocket, live candle, bid/ask depth) — v16.0

### Active

(Defining requirements for v23.0)

## Previous Milestone: v22.0 Platform Polish & AI Coverage (SHIPPED)

**Delivered:** Data quality fixes (499 tests passing), AI analysis auto-coverage for full watchlist with scheduled runs and coverage dashboard, simulator enhancement (auto-sell on SL/TP hit, AI sell signal integration, equity curve chart, P&L timeline), UX polish (skeleton loading states, mobile responsive, CSS page transitions, homepage redesign with portfolio metrics + AI signals). Fixed SSL certificate bypass for VCI/KBS/vnstocks domains, resolved Aiven DB disk quota exhaustion. 4 phases, 9 plans.

## Previous Milestone: v21.0 Stock Trading Simulator (SHIPPED)

**Delivered:** Paper trading simulator with AI-driven daily picks, auto-trade execution, FIFO lot matching, fee calculation (0.15% broker + 0.1% sell tax), portfolio tracking with unrealized P&L, and full stats dashboard. Fixed pick generation pipeline for unified AI format (Vietnamese signal names, unified analysis type). 1 phase, 4 plans.

## Previous Milestone: v20.0 Enhanced Price Pipeline (SHIPPED)

**Delivered:** VCI polling fallback (VNDirect WS domain dead), real-time price persistence to DB, auto-crawl on market close, improved polling frequency. 3 phases.

## Previous Milestone: v19.0 Unified AI Analysis Pipeline (SHIPPED)

**Delivered:** Unified 5 separate AI analysis types (technical, fundamental, sentiment, combined, trading_signal) into a single Gemini prompt producing one coherent recommendation with entry/SL/TP/reasoning. Frontend redesigned with unified panel as primary view. Fixed division-by-zero in volume trend calculation. Improved reasoning prompt for structured 4-section output (500+ chars). 4 phases.

## Previous Milestone: v18.0 Multi-Source Community Rumors (SHIPPED)

**Delivered:** Fixed all 6 rumor crawlers (SSL verify=False, TNCK rewrite to RSS, NhaDauTu rewrite to HTML scraping), verified multi-source AI scoring pipeline, fixed trading signal validation price unit mismatch (DB stores ×1000 VND, Gemini outputs actual VND). 4 phases.

## Previous Milestone: v17.0 AI Consistency & UX (SHIPPED)

**Delivered:** Fixed AI combined prompt for consistency, trading signal dual-direction with recommended direction display, VNDirect WebSocket connectivity fix for Render. 3 phases.

## Previous Milestone: v16.0 Real-Time Price (SHIPPED)

**Delivered:** VNDirect WebSocket client connecting to `wss://price-cmc-04.vndirect.com.vn/realtime/websocket` with SP/BA message parsing, exponential backoff reconnect (1s→60s cap), market hours enforcement. Backend broadcasts price updates to frontend via existing ConnectionManager with snapshot-on-subscribe, dynamic symbol sync, and bid/ask broadcasting. Frontend live candlestick bar updates via realtimePrice prop, BidAskPanel with 3 bid/3 ask levels and volume bars. Feature flag `vndirect_ws_enabled` for instant VCI polling fallback. 4 phases, 3 plans.

## Previous Milestone: v15.0 Performance Optimization (SHIPPED)

**Delivered:** Database composite indexes on 5 hot tables, connection pool tuning (10+10, recycle 1800s), N+1 query fixes (rumor batch aggregates, AI context builder window functions), pagination for watchlist/rumor endpoints, TTLCache on 5 API endpoints, parallel crawler fetch (Semaphore-bounded Fireant×3, CafeF×5), concurrent RSS crawlers (F319+VnExpress+Vietstock via asyncio.gather), bulk INSERT ON CONFLICT for all 5 crawlers + financials, centralized ticker map per job run, CPU parsing offloaded to asyncio.to_thread(). 5 phases, 6 plans.

## Previous Milestone: v14.0 Multi-Source Rumor & Quota Fix (SHIPPED)

**Delivered:** Switched Gemini model to gemini-2.5-flash (from gemini-2.5-flash-lite), fixed httpx AsyncClient monkey-patch recursion bug, added VnExpress RSS crawler (kinh-doanh feed), Vietstock multi-feed RSS crawler (5 feeds with cross-feed dedup), integrated all sources into scoring pipeline with credibility tiers, source-aware prompts grouping official vs community posts. 3 phases, 3 plans.
...
---
*Last updated: 2026-05-13 after starting v22.0 milestone*