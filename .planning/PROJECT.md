...
(v15.0 — see Current Milestone below)

## Current Milestone: v15.0 Performance Optimization

**Goal:** Tối ưu performance toàn diện — DB indexes, N+1 query fixes, API caching, crawler parallelization, và bulk operations.

**Target features:**
- Database indexes cho 7 tables trên hot query columns
- Fix N+1 queries (rumor summary, AI context builder)
- API response caching (TTLCache) cho expensive endpoints
- Parallel crawlers với bounded concurrency + bulk inserts
- DB pool tuning + pagination cho list endpoints
- Async-safe patterns (CPU parsing → thread pool)
- Financial service bulk upsert
