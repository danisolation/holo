"""TuoiTre RSS crawler for stock news/rumors.

Fetches stock news from Tuoi Tre RSS feed (chung-khoan.rss).
Covers VN-Index movements, AGM reports, insider trades.

pubDate uses non-standard "GMT+7" timezone wrapped in CDATA.
Title, link, description, pubDate are all CDATA-wrapped.
GUID format: UUID with isPermaLink="false".
"""
import asyncio
import hashlib
import html
...
RSS_URL = "https://tuoitre.vn/rss/chung-khoan.rss"
...
def _parse_gmt7_date(date_str: str) -> datetime:
    ...
    TuoiTre uses "Thu, 07 May 2026 15:33:55 GMT+7" format,
    wrapped in CDATA. Normalize to RFC 2822 before parsing.
    """
...
class TuoiTreCrawler:
    """Crawls TuoiTre RSS feed for stock-relevant news."""
    ...
    async def crawl_rss(self, *, ticker_map: dict[str, int] | None = None) -> RumorCrawlResult:
        """Fetch TuoiTre RSS, extract ticker-tagged articles, store in rumors.
        ...
        if not ticker_map:
            logger.warning("TuoiTre crawler: no watchlist tickers found")
            return {"success": 0, "failed": 0, "total_posts": 0, "failed_symbols": []}

        watchlist_symbols = set(ticker_map.keys())
        logger.info(f"Starting TuoiTre RSS crawl (watchlist: {len(watchlist_symbols)} tickers)")
        ...
        for item in items:
            ...
            for symbol in matched:
                ...
                post_id = _guid_to_post_id(f"tuoitre:{item['guid']}:{symbol}")
        ...
        logger.info(
            f"TuoiTre RSS crawl complete: {stored} posts stored for "
            f"{len(tickers_found)} tickers ({sorted(tickers_found)})"
        )
        return result_dict

    def _parse_rss(self, xml_text: str) -> list[dict]:
        """Parse TuoiTre RSS XML into list of article dicts."""
        try:
