...
def configure_jobs():
    ...
    # ── Phase 114: Health monitoring with Telegram alerts ────────────────
    from app.services.health_monitor_service import ping_health

    scheduler.add_job(
        ping_health,
        trigger=IntervalTrigger(minutes=settings.health_ping_interval_minutes),
        id="health_monitor_ping",
        name="Health Monitor Self-Ping",
        replace_existing=True,
        misfire_grace_time=120,
    )
    logger.info(f"Health monitor: self-ping every {settings.health_ping_interval_minutes} minutes with Telegram alerts")
