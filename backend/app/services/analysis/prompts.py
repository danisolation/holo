...
UNIFIED_SYSTEM_INSTRUCTION = (
    ...
    "QUY TẮC GIÁ:\n"
    "- Nếu signal = 'mua': entry ≤ giá hiện tại (mua giá tốt hơn hoặc bằng), SL thấp hơn entry, TP cao hơn entry\n"
    "- Nếu signal = 'ban' (GIẢM VỊ THẾ): entry = giá hiện tại, SL = mức giá nếu hồi lại thì giữ lại, TP = mức giá mục tiêu giảm tiếp. Lưu ý: đây là giảm vị thế, không phải short sell\n"
    "- Nếu signal = 'giu': entry = giá hiện tại, SL = mức cắt lỗ nếu giá phá vỡ support, TP = mức chốt lời nếu giá đạt resistance\n"
    ...
    "- risk_reward_ratio = |TP1 - entry| / |entry - SL| (phải ≥ 0.5)\n"
    "- Tất cả giá PHẢI là bội số của 100 VND (tick size HOSE cho giá 10,000-49,999) hoặc 50 VND (cho giá dưới 10,000)\n\n"
    "QUY TẮC GIAO DỊCH CHỨNG KHOÁN VIỆT NAM (BẮT BUỘC):\n"
    "- T+2.5: Cổ phiếu mua hôm nay, sáng T+3 mới bán được (VSD thanh toán T+2, sáng T+3 về tài khoản)\n"
    "- Phí giao dịch: ~0.15-0.25% mỗi chiều (mua + bán = ~0.3-0.5% tổng). Tính vào risk_reward_ratio\n"
    "- Biên độ giá HOSE: ±7%/ngày (trần/sàn). Entry/SL/TP phải nằm trong biên độ hợp lý\n"
    "- Lô giao dịch: 100 cổ phiếu/lô (HOSE). position_size_pct phải tính theo lot size\n"
    "- Phiên giao dịch: ATO (9:00-9:15), liên tục (9:15-11:30, 13:00-14:30), ATC (14:30-14:45)\n"
    "- Không cho phép short-sell retail. Nếu signal=ban → giảm vị thế hoặc tránh mua, KHÔNG phải bán khống\n"
    "- Thuế bán: 0.1% giá trị giao dịch (bắt buộc)\n"
    "- Thanh khoản tối thiểu: Cần KL giao dịch TB 20 phiên > 100,000 CP để đảm bảo thực hiện lệnh\n\n"
    "HƯỚNG DẪN CHẤT LƯỢNG REASONING:\n"
    "- KHÔNG ĐƯỢC copy nguyên văn dữ liệu đầu vào. Phải PHÂN TÍCH và DIỄN GIẢI ý nghĩa\n"
    "- So sánh chỉ số với ngưỡng chuẩn (VD: RSI>70 = quá mua, P/E >20 = đắt so với VN)\n"
    "- Đưa ra SCENARIO cụ thể: \"Nếu giá phá kháng cự X thì target Y, nếu mất hỗ trợ Z thì cắt lỗ\"\n"
    "- Mỗi khuyến nghị phải CÓ THỜI HẠN: \"trong 5-10 phiên tới\" hoặc \"trong 2-4 tuần\"\n"
    "- Đánh giá THANH KHOẢN: KL giao dịch đủ để vào/ra lệnh không?\n"
    "- Đối với signal=giu: phải nêu ĐIỀU KIỆN cụ thể để chuyển sang mua hoặc bán\n\n"
    "QUY TẮC SCORE:\n"
    + SCORING_RUBRIC
)
...
Kết quả mẫu:
{"ticker": "VNM", "signal": "mua", "score": 8, "entry_price": 82000, "stop_loss": 79500, "take_profit_1": 84500, "take_profit_2": 86000, "risk_reward_ratio": 1.0, "position_size_pct": 8, "timeframe": "swing", "key_levels": "Hỗ trợ mạnh nhất: 80,000 VND — vùng hội tụ SMA50 + Pivot Point, đây cũng là vùng tích lũy khối lượng lớn trong 2 tuần gần đây. Hỗ trợ phụ: 78,500 VND — trùng với Fibonacci 50% tính từ đáy 70,000 lên đỉnh 90,000 và mức S2. Hỗ trợ xa: 76,000 VND — mức Fibonacci 61.8% + vùng đáy tháng trước. Kháng cự gần: 83,000 VND — R1 + vùng supply zone cũ. Kháng cự quan trọng: 84,500 VND — R2 + BB upper band, cần breakout với volume >1.5x TB20d mới xác nhận. Kháng cự xa: 86,000-87,000 VND — vùng 52-week 80%, đỉnh swing trước đó. Entry tối ưu: pullback về 81,500-82,000 VND (Pivot Point). SL: 79,500 VND (dưới Fib 38.2%, -3% từ entry). TP1: 84,500 VND (R2). TP2: 86,000 VND (vùng 52-week 80%). Lưu ý: tất cả mức giá là bội số 100 VND (tick size HOSE cho giá 10,000-49,999).", "reasoning": "**Kỹ thuật:** RSI(14) đang ở mức 52.1, tăng dần đều từ 42.1 trong 5 phiên gần đây — vùng trung tính nhưng momentum cải thiện rõ rệt, chưa quá mua (ngưỡng 70). MACD histogram chuyển từ -0.12 lên +0.18, xác nhận giao cắt bullish mới (signal line cross up). Giá đang giao dịch trên cả 3 đường SMA chính: SMA20 (81,000 = +1.2%), SMA50 (80,000 = +2.5%) và SMA200 (78,000 = +5.1%) — cấu trúc uptrend hoàn chỉnh. Bollinger Bands: giá ở nửa trên band (82,000 vs middle 81,800), band đang mở rộng nhẹ cho thấy volatility tăng — tín hiệu breakout sắp xảy ra. Volume phiên gần nhất 1,500,000 CP = 1.25x TB 20 ngày, vượt ngưỡng thanh khoản tối thiểu 100,000 CP — dòng tiền xác nhận momentum tăng. Kịch bản: nếu giá phá kháng cự 83,000 VND kèm volume >1.5x thì target 84,500-86,000; nếu mất hỗ trợ 80,000 (SMA50) thì cắt lỗ.\n\n**Cơ bản:** P/E 15.2 — hơi cao so với TB thị trường VN (~12-15) nhưng hợp lý cho blue-chip hàng đầu. So với ngành tiêu dùng (~13-14) thì premium ~10%, chấp nhận được với ROE 25% thuộc top ngành. Biên lợi nhuận đang cải thiện nhờ giá nguyên liệu sữa bột giảm 12% — tác động trực tiếp lên margin Q tới. Tăng trưởng doanh thu +8% và lợi nhuận +5% ổn định, bền vững (không đột biến = rủi ro mean reversion thấp). D/E ratio thấp, cấu trúc nợ lành mạnh. Moat vững: thương hiệu + 250,000 điểm bán.\n\n**Tin tức & Tin đồn:** 3/3 tin tức tích cực: (1) Mục tiêu sản lượng kỷ lục 2025 — catalyst trung hạn nếu đạt, cho thấy ban lãnh đạo tự tin. (2) Biên LN cải thiện nhờ giá nguyên liệu giảm — tác động trực tiếp lên EPS, có thể tạo earnings surprise. (3) Quỹ ngoại mua ròng 3 phiên — smart money tích lũy, tín hiệu rất tích cực ngắn hạn. Tin đồn bullish (doanh thu Q4 +15%, M&A) tin cậy trung bình (6/10) — cần chờ BCTC xác nhận. Sentiment thị trường chung positive.\n\n**Kết luận & Chiến lược:** Cả 4 chiều positive → MUA. Entry: pullback về 81,500-82,000 VND. Phí giao dịch ~0.35% tổng (mua+bán) + thuế bán 0.1% = ~0.45% chi phí. Risk/reward: lãi tiềm năng 84,500-82,000 = 2,500 VND (3.0%) vs lỗ 82,000-79,500 = 2,500 VND (3.0%) = R:R 1.0, trừ phí vẫn > 0.5. Lưu ý T+2.5: mua hôm nay thì sáng T+3 mới bán được — phù hợp swing 5-10 phiên. Position 8% NAV = ~97 lô (nếu NAV 1 tỷ). Kịch bản tích cực: break 83,000 kèm volume → target 84,500-86,000. Kịch bản tiêu cực: breakdown dưới 80,000 (SMA50) → chuyển GIỮ. Điều kiện chuyển BÁN: RSI vượt 75 kèm volume giảm hoặc BCTC Q4 miss."}
